...
이 글은 그 이야기이기도 하지만, 시작은 좀 더 단순했습니다.

**오퍼레이터를 한 번 만들어보고 싶었습니다.** 컨트롤러를 쓰는 쪽으로는 익숙한데 만드는 쪽은 해본 적이 없었고, 플랫폼 엔지니어링 영역에 직접 손을 대보고 싶었습니다.

마침 고칠 만한 게 하나 있었습니다.

---

## 1. Lambda가 나빠서가 아니라, 우리가 안 쓰는 도구라서

dev 환경 야간·주말 스케일다운은 EventBridge + Lambda로 만들어 두고 잘 돌고 있었습니다. 문제는 동작이 아니었습니다.

**평소에 Lambda를 쓰지 않습니다.** 인프라 대부분이 쿠버네티스와 Terraform 위에 있는데, 이 기능 하나 때문에 관리해야 할 곳이 하나 더 생겼습니다.

그러면 이런 것들이 애매해집니다.

- 스케줄을 바꾸려면 어디를 고쳐야 하는가
- 그 변경은 어떤 경로로 배포되는가
- 다른 사람이 인수인계받으면 어디부터 봐야 하는가

동작하는 코드보다 **누가 어떻게 유지보수할 것인가**가 걸렸습니다. 쿠버네티스 생태계 안에 있는 것들은 배포도 조회도 권한도 같은 방식으로 다루는데, 이것만 바깥에 있었습니다.

기술적인 한계도 물론 있었습니다.

**클러스터 안에서 무슨 일이 일어나는지 모릅니다.** Lambda는 정해진 시각에 replica를 0으로 바꿉니다. 그 순간 누가 장애 대응 중이라 파드를 켜놨는지, HPA가 붙어 있는 워크로드인지 알지 못합니다.

**설정이 개발자에게 보이지 않습니다.** 스케줄이 Lambda 코드나 환경변수 안에 있어서, 개발자는 자기 네임스페이스가 몇 시에 꺼지는지 확인할 방법이 없습니다.

**그리고 한 번 쏘고 끝입니다.** 20시에 실패하면 그날은 그냥 안 됩니다.

이 정도면 오퍼레이터로 옮겨서 간소화할 수 있겠다 싶었고, 만들어보면서 실제로 어떤 이점이 생기는지도 확인해보고 싶었습니다.
