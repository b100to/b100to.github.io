...
description: "k9s views.yaml 커스텀 컬럼 설정으로 Karpenter NodePool별 노드 용도(app, worker, gpu 등)를 한눈에 파악하는 방법을 공유합니다."
...
어떤 노드가 app이고, 어떤 게 worker인지 전혀 알 수가 없습니다. IP 기반 Private DNS가 노드 이름이라 의미 있는 정보가 없기 때문입니다.
...
NodePool별로 `node-role: app`, `node-role: worker`, `node-role: gpu` 이런 식으로 설정해두면 됩니다.
...
`.metadata.labels.node-role` 경로로 라벨 값을 추출해서 `TYPE`이라는 컬럼으로 보여줍니다.
...
노드를 선택해서 들어가면 상단에 `pods(ip-10-0-xx-xxx.ap-northeast-2.compute.internal)` 형태로 표시되는데, 이건 Kubernetes 리소스 이름을 그대로 사용하는 부분이라 커스터마이징이 안 됩니다. `views.yaml`은 리스트 뷰의 컬럼만 제어할 수 있고, 드릴다운 뷰의 헤더 타이틀은 k9s에 하드코딩되어 있습니다.
