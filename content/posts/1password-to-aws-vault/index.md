...
date: 2026-01-28T15:06:00+09:00
description: "1Password credential_process의 팝업 문제로 aws-vault로 전환한 경험을 공유합니다. k9s, Lens 같은 도구 사용 시 팝업 지옥에서 벗어난 방법."
keywords: ["AWS", "aws-vault", "1Password", "credential_process", "DevOps", "EKS", "k9s", "Lens"]
categories: ["AWS"]
tags: ["AWS", "aws-vault", "1Password", "Security", "DevOps", "EKS"]
showHero: true
...
[지난 글](/posts/aws-credentials-1password)에서 `~/.aws/credentials`의 평문 키를 1Password `credential_process`로 대체했다. 보안은 확실히 강화됐는데...
...
처음엔 "보안을 위해서니까" 하고 참았다.

### k9s 사용 시 진짜 문제

문제는 k9s, Lens 같은 도구였다.

EKS 클러스터에 접근하면 kubeconfig에서 `aws eks get-token`이 실행된다:
...
1Password CLI의 권한 팝업은 **비활성화가 불가능**했다.

- 공식 문서: 언급 없음
- 1Password 커뮤니티: "보안 설계상 의도된 동작"
- GitHub Issues: 해결책 없음

10분 내 재요청 시 팝업 스킵된다고 하는데, 새 터미널 탭/창 열면 무조건 다시 뜬다.
...
`credential_process` 라인 삭제. aws-vault가 환경변수로 자격증명을 주입한다.
...
서브쉘 안에서는 `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN` 환경변수가 설정되어 있어서 모든 AWS 도구가 팝업 없이 작동한다.
...
서브쉘 안에서 다른 프로필로 전환이 안 된다. 반드시 `exit` 후 재진입해야 한다.
...
각 탭에서 다른 환경 서브쉘을 유지하면 전환 없이 작업 가능하다.
...
나는 **aws-vault**로 정착했다. 터미널 탭 분리로 계정 전환 불편함을 해결하니, 팝업 없는 쾌적함이 압도적으로 좋았다.
